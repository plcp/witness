...
class test_case(unittest.TestCase):
    ...
    def test_evidences(self):
        ...
        self.assertTrue(local_uid + 1 == wit.fuzzy._local_occuring_uid)

        # check if the uuid's first field is magic
        self.assertTrue(uuid_tcheck.startswith(wit.fuzzy.uuid_magic))
        uuid_ncheck = uuid_tcheck[len(wit.fuzzy.uuid_magic) + 1:]

        # check if the uuid's second field is the correct api version
        self.assertTrue(uuid_ncheck.startswith('%x%x%02x' % wit.api_version))
        uuid_ncheck = uuid_ncheck[5:]

        # check if the uuid's third field is equal to the « session » token
        self.assertTrue(uuid_ncheck.startswith(wit.fuzzy.uuid_session))
        ...
        self.assertTrue(isinstance(e.value, wit.logic.tbsl))
        self.assertTrue(wit.logic.obsl(size=37) == e.value)
